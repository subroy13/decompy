...
def build_action():
    ...
    if os.path.exists(os.path.join(project_root, "dist")) and os.path.isdir(
...
def document_action():
    ...
    subprocess.Popen(
        "sphinx-apidoc -o docsource/ -d 3 ./src",
    ...
    subprocess.Popen(
        "make clean", cwd=os.path.join(project_root, "docsource"), shell=True
    ).wait()
    subprocess.Popen(
        "make html", cwd=os.path.join(project_root, "docsource"), shell=True
    ).wait()

    # copy all the files from docsource to docs
    if os.path.exists(os.path.join(project_root, "docs")) and os.path.isdir(
        os.path.join(project_root, "docs")
    ):
        shutil.rmtree(os.path.join(project_root, "docs"))
    # copy html files
    shutil.copytree(
        os.path.join(project_root, "docsource", "_build", "html"),
        os.path.join(project_root, "docs"),
    )

    # server the server
    subprocess.Popen(
        f"{python_cmd} -m http.server",
...
def deploy_action():
    ...
    subprocess.Popen(
        "twine upload dist/* --verbose", cwd=project_root, shell=True
    ).wait()
...
def test_action():
    ...
    subprocess.Popen(
        "pytest --cov-report term-missing --cov=decompy tests/",
        cwd=project_root,
        shell=True,
    ).wait()
